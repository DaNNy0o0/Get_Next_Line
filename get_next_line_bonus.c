...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/10 18:04:55 by ddiaz-to          #+#    #+#             */
/*   Updated: 2024/06/10 18:04:55 by ddiaz-to         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		...
		if (bytes <= 0)
			break ;
		buffer[bytes] = '\0';
		store[fd] = join_read_data(store[fd], buffer);
		if (find_new_line(store[fd]) >= 0)
			break ;
	}
	free(buffer);
	if (bytes < 0 || !store[fd] || !*store[fd])
		return (free(store[fd]), store[fd] = NULL, NULL);
	return (divide_strings(&store[fd]));
}
